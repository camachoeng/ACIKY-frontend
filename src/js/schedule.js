...
export async function initSchedule() {
  ...
  function updateWhatsAppCta() {
    ...
    if (ctaBtn) {
      ...
      ctaBtn.href = buildWhatsAppUrl(waPhone, message)
    }
  }
  ...
  container.addEventListener('click', (e) => {
    ...
    const user = getUser()
    if (!user) {
      // Redirect to login with return URL and reason
      const returnUrl = encodeURIComponent(window.location.pathname + window.location.search)
      window.location.href = import.meta.env.BASE_URL + 'pages/login.html?reason=booking&return=' + returnUrl
      return
    }

    // User is logged in, open WhatsApp
    const whatsappUrl = btn.dataset.book
    ...
    <div class="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100">
      ...
      <div class="mt-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <button data-book="${buildWhatsAppUrl(waPhone, bookMessage)}"
